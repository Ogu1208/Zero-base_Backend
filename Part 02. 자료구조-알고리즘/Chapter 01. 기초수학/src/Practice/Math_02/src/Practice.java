package Practice.Math_02.src;// Practice
...
class MySet {
    ...
    public MySet addAll(MySet b) {
        MySet result = new MySet();   // 새로운 것으로 반환

        for (int itemA : this.list) {
            result.add(itemA);    // add는 중복 x 추가
        }
    ...
    public MySet removeAll(MySet b) {
        ...
        for (int itemA : this.list) {
            ...
            for (int itemB : b.list) {   // 같은 원소가 있으면 for문 탈출
                if (itemA == itemB) {
                    containFlag = true;
                    break;
                }
            ...
            if(!containFlag) {  // Flag가 false이면 추가
                result.add(itemA);
            }
        }
